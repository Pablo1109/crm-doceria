...
export default function FinanceiroClient({ transactions, orderCosts }: FinanceiroClientProps) {
  ...
  const activePeriodData = useMemo(() => {
    ...
    // 1. Filtrar transações financeiras
    const periodTransactions = transactions.filter(t => 
      t.date >= startDate && t.date <= endDate
    );

    // 2. Filtrar custos de pedidos da quinzena
    const periodOrderCosts = orderCosts.filter(o => 
      o.date >= startDate && o.date <= endDate
    );

    // Cálculos de Caixa (Cofre)
    ...
    // Lançamentos no Cofre acumulados de sempre (Saldo Real no cofre físico)
    const totalHistoryIncome = transactions.filter(t => t.type === "income").reduce((s, t) => s + Number(t.amount), 0);
    const totalHistoryExpense = transactions.filter(t => t.type === "expense").reduce((s, t) => s + Number(s + Number(t.amount)), 0);
    const totalHistoryWithdrawal = transactions.filter(t => t.type === "withdrawal").reduce((s, t) => s + Number(t.amount), 0);
    
    // O saldo físico do cofre considera todos os tempos
    ...
    // Cálculos Operacionais (Lucro Real)
  ...
  }, [selectedFortnight, transactions, orderCosts]);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between bg-white border border-[#ead8cf] p-4 rounded-3xl shadow-sm">
        ...
        >
          <Plus className="h-4.5 w-4.5" /> Lançar Movimentação
        </button>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
        ...
        <div className="card-soft rounded-[2.5rem] p-6 bg-gradient-to-b from-slate-800 to-slate-950 border border-slate-700 shadow-2xl flex flex-col justify-between min-h-[380px] text-white relative overflow-hidden">
          ...
          <div className="z-10 flex justify-between items-start">
            ...
            <button 
              ...
              className={`p-3 rounded-2xl cursor-pointer shadow-md transition-all flex items-center justify-center ${safeOpen ? "bg-[#efb7c8] text-slate-900 animate-pulse" : "bg-slate-700 text-slate-300 hover:bg-slate-600"}`}
            >
          ...
          <div className="my-8 flex justify-center items-center relative z-10">
            {!safeOpen ? (
              ...
              <div className="text-center animate-toast-in space-y-4">
                ...
                <div>
                  <p className="text-xs font-black uppercase tracking-widest text-slate-400">Saldo Atual no Cofre</p>
                  ...
                  <p className="text-[10px] text-slate-500 mt-2 font-bold leading-relaxed max-w-xs mx-auto">
                    Este valor representa o dinheiro físico real acumulado de todas as entradas concluídas menos compras e retiradas.
                  </p>
                </div>
              </div>
            )}
        ...
        <div className="card-soft rounded-[2.5rem] p-6 bg-white border border-[#ead8cf] flex flex-col justify-between shadow-sm">
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-6">
              <div className="rounded-2xl bg-slate-50 p-4 border border-slate-100">
                <div className="flex items-center gap-2 text-emerald-600">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">Entradas</span>
                </div>
                <p className="mt-1.5 text-xl font-black text-slate-900">{money(activePeriodData.income)}</p>
                <span className="text-[9px] text-slate-400 font-bold">Vendas entregues</span>
              </div>

              <div className="rounded-2xl bg-slate-50 p-4 border border-slate-100">
                <div className="flex items-center gap-2 text-rose-500">
                  <TrendingDown className="h-4.5 w-4.5" />
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">Saídas</span>
                </div>
                <p className="mt-1.5 text-xl font-black text-slate-900">{money(activePeriodData.expense)}</p>
                <span className="text-[9px] text-slate-400 font-bold">Estoque e custos fixos</span>
              </div>
            </div>

            {/* Pró-labore lançado na quinzena */}
            <div className="mt-4 flex justify-between items-center rounded-2xl bg-violet-50/50 p-4 border border-violet-100 text-sm">
            ...
            {/* Diferença Caixa vs Operacional */}
            <div className="mt-6 border-t border-slate-100 pt-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-base font-black text-[#5b382d]">{money(activePeriodData.opRevenue)}</p>
                </div>
                <div>
                  <p className="text-[10px] font-bold text-slate-500">Custo Real de Ingredientes (COGS)</p>
                  <p className="text-base font-black text-[#5b382d]">{money(activePeriodData.opIngredientsCost)}</p>
                </div>
              </div>

              <div className="flex justify-between items-center bg-[#fff8ef] rounded-2xl p-4 border border-[#ead8cf] shadow-inner">
                <div>
                  <p className="text-xs font-bold text-[#8b6a5d]">Lucro Operacional Estimado</p>
                  <p className="text-2xl font-black text-[#5b382d] mt-0.5">{money(activePeriodData.opProfit)}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs font-bold text-[#8b6a5d]">Margem de Lucro</p>
                  <p className="text-2xl font-black text-rose-500 mt-0.5">{activePeriodData.opMargin.toFixed(1)}%</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="card-soft overflow-hidden rounded-[2rem] bg-white border border-[#ead8cf]">
        <div className="border-b border-[#ead8cf] bg-[#fff8ef] p-5">
          <h2 className="text-lg font-black text-[#5b382d]">Livro de Caixa da Quinzena</h2>
          <p className="text-xs text-[#8b6a5d] mt-1 font-bold">Todas as movimentações ocorridas no período selecionado.</p>
        </div>
              ...
              {activePeriodData.periodTransactions.map(t => (
                <tr key={t.id} className="hover:bg-[#fff1f4]/40 text-xs">
                  ...
                  <td className="px-6 py-4 text-right">
                    {/* Não permitir deletar direto se for automático de pedido ou estoque, a não ser que queiram */}
                    {t.category === "pedido" || t.category === "ingrediente" ? (
                      <span className="text-[10px] text-slate-400 font-bold" title="Lançamentos automáticos devem ser excluídos na aba de Pedidos ou Estoque.">Automático</span>
                    ) : (
                      <form action={async () => {
              ...
              {activePeriodData.periodTransactions.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-slate-400 italic">
                    Nenhuma movimentação financeira lançada nesta quinzena.
                  </td>
                </tr>
              )}
            </tbody>
